...
static jintArray native_spawn_impl(JNIEnv *env, jobjectArray java_argv,
    ...
    if (pid == 0) {
        ...
        dprintf(STDERR_FILENO, "LinuxDroid native exec failed: %s\n", strerror(errno));
        _exit(127);
    }
...
static jint native_kill_impl(jint pid, jint signal) {
    ...
    // wrapper pid if group creation raced with an early exit.
...
JNIEXPORT jintArray JNICALL
Java_com_linuxdroid_runtime_NativeSpawn_spawn(JNIEnv *env, jobject self, jobjectArray java_argv,
                                              jobjectArray java_env, jstring java_cwd,
                                              jstring java_output) {
    (void)self;
    return native_spawn_impl(env, java_argv, java_env, java_cwd, java_output);
...
JNIEXPORT jint JNICALL
Java_com_linuxdroid_runtime_NativeSpawn_waitFor(JNIEnv *env, jobject self, jint pid, jboolean no_hang) {
    (void)env; (void)self;
    return native_wait_for_impl(pid, no_hang);
}

JNIEXPORT jint JNICALL
Java_com_linuxdroid_runtime_NativeSpawn_kill(JNIEnv *env, jobject self, jint pid, jint signal) {
    (void)env; (void)self;
    return native_kill_impl(pid, signal);
}

// Mobile-Harness backwards-compatibility JNI exports
JNIEXPORT jintArray JNICALL
